...
for counter in range(int(number_pods)):
    ...
    for access_point in output_json["AccessPoints"]:
        if access_point["RootDirectory"]["Path"] == f'/{namespace}/file-storage-{helm_installation_name}-sumologic-otelcol-logs-{counter}':
            fsap_id = access_point["AccessPointId"]
            break
    else:
        cmd = f'aws efs create-access-point --file-system-id {efs_id} --posix-user Uid=1000,Gid=1000 --root-directory "Path=/{namespace}/file-storage-{helm_installation_name}-sumologic-otelcol-logs-{counter},CreationInfo={{OwnerUid=1000,OwnerGid=1000,Permissions=777}}" --region {aws_region}'
        output = subprocess.check_output(cmd, shell=True)
